    ...
    return(
        <div className="w-full lg:w-96 h-[26rem] flex flex-col gap-4 bg-base-100 rounded-md p-8 capitalize">
            <div className="flex flex-col gap-1 items-center justify-center">
                <GiKoala className="text-5xl text-primary" />
                <h1 className="font-semibold text-3xl">Lupa Password</h1>
                <span className="text-sm font-sans text-center">Harap masukan Username / email anda</span>
            </div>
            <form className="flex flex-col gap-1">
                <div className="w-full">
                    <input type="text" name="username" placeholder="Username" className={`input input-primary w-full ${showCode ? 'hidden' : ''}`} value={state.username} onChange={handleInputChange} />
                </div>
                {showCode ? 
                    <>
                        <span className="text-center text-xl lowercase">{showCode}</span>

                        <input type="text" name="code" placeholder="Masukan Kode Di Atas" className={`input input-primary w-full`} value={state.code} onChange={handleInputChange} />
                        <div className="w-full">
                ...
                <button onClick={handleSubmit} type="submit" className="btn btn-primary w-full" >submit</button>
            </form>
